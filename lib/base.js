...
async function smsDelete(ids) {
  ...
  if (!result) {
    log("smsDelete: failed to delete sms.", ids);
  } else {
    log(result);
    log("smsDelete: success to delete sms.", ids);
  }
}
...
async function smsRead(msgId) {
  ...
  const result = await sendRequest("message", body);
  if (!result) {
    log("smsRead: failed to read sms.", msgId);
  } else {
    log(result);
    log("smsRead: success to read sms.", msgId);
  }
}
...
async function smsCheck() {
  ...
  if (unreadNum == 0) {
    log("smsCheck: (status) no unread messages.");
    return;
  }
...
async function sendWX(title, desp) {
  const url = new URL(config.WX_REPORT_URL);
  ...
  try {
    ...
    log("sendWX res:", res.status);
    return true;
  } catch (error) {
    log("sendWX error:", error);
    return false;
  }
}
...
async function sendTG(title, desp) {
  ...
  const url = new URL(config.TG_REPORT_URL);
  ...
  try {
    ...
    const text = await res.text();
    log("sendTG res:", res.status);
    return true;
  } catch (error) {
    log("sendTG error:", error);
    return false;
  }
}
...
async function main() {
  // await sendWX("hellowx", "wxhaha" + new Date());
  // await sendTG("hellotg", "telehaha" + new Date());
  await smsCheck();
  // await smsRead("LRCV9");
  // await smsDelete("LRCV9");
